	...
	public class GameUI
	{
		...
		public void NewGame()
		{
			...
			if (NewGameDlg.bStartGame)
			{
				...
				{
					ChessGame.WhitePlayer.TotalThinkTime = 30;	// set maximum thinking time
					ChessGame.BlackPlayer.TotalThinkTime = 30;	// set maximum thinking time
				}

				// Chess Champion
				if (NewGameDlg.PlayerLevel4.Checked)
                {
					ChessGame.WhitePlayer.TotalThinkTime = 45;  // set maximum thinking time
					ChessGame.BlackPlayer.TotalThinkTime = 45;  // set maximum thinking time
				}

				// Chess Grand Champion
				if (NewGameDlg.PlayerLevel5.Checked)
                {
					ChessGame.WhitePlayer.TotalThinkTime = 60; // set maximum thinking time
					ChessGame.BlackPlayer.TotalThinkTime = 60; // set maximum thinking time
				}
				InitPlayers();
				RedrawBoard();		// Make the chess board visible on screen
				NextPlayerTurn();		// When the both players are computer this start the game 
